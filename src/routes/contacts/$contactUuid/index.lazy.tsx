...
function Contact() {
  ...
  const { isPending, isError, data, error } = useGetUserByIdQuery(contactUuid)

  const mutation = useMutation({
    mutationFn: (contactUuid: string) => deleteUser(contactUuid),
    onSuccess: () => {
  ...
  return data ? (
    <div className="flex items-center p-4 space-x-4 max-w-md">
      ...
      <div className="flex-1">
        ...
        <a href={data.socialMedia} target="_blank" className="text-blue-500">
          {data.socialMedia}
        </a>
        <p className="text-gray-500">{data.description}</p>
        <div className="flex space-x-2 mt-2">
          <Link
              to={`/contacts/${contactUuid}/edit`}
